...
def radarr_change_root_directories(your_database, replace_this, with_this):
    ...
    if os.path.exists(your_database):
        ...
        # NetImport, RootFolderPath column
        ...
        # then you can use these two blocks to set the RootFolderPath on all NetImports simultaneously
        # for row in c.execute('SELECT * FROM NetImport WHERE RootFolderPath Like "%"'):
        for row in c.execute('SELECT * FROM NetImport WHERE RootFolderPath Like "' + str(replace_this) + '%"'):
            net_import_to_change.append(row[7])  # Append RootFolderPath column to list

        for item in net_import_to_change:
            print(item)
            new_path = re.sub(replace_this, with_this, item)
            # c.execute('UPDATE NetImport SET RootFolderPath="' + with_this + '" WHERE RootFolderPath like "%"')
            c.execute('UPDATE NetImport SET RootFolderPath="'+new_path+'" WHERE RootFolderPath like "'+item+'%"')
...
# Apply to sonarr
sonarr_change_root_directories(sonarr_db, sonarr_root_directory_to_replace, sonarr_root_directory_to_replace_with)

# Apply to radarr
radarr_change_root_directories(radarr_db, radarr_root_directory_to_replace, radarr_root_directory_to_replace_with)
